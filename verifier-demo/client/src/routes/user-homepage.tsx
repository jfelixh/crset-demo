...
import { CartesianGrid, Line, LineChart, XAxis, YAxis, Bar, BarChart, Cell, LabelList, Pie, PieChart  } from "recharts";
import {
  ChartConfig,
  ChartContainer,
  ChartTooltip,
...
import { EuroIcon, Hand, HandCoinsIcon, CreditCardIcon, TargetIcon, WalletCardsIcon, PiggyBankIcon, BadgeEuroIcon, UserRoundCogIcon } from 'lucide-react';
import { createFileRoute, Link } from '@tanstack/react-router'
import { Separator } from '@/components/ui/separator';
import { Button } from '@/components/ui/button';

export const Route = createFileRoute('/user-homepage')({
...
function RouteComponent() {
  ...
  const AI = IE.map((item) => {return {month: item.month, income: item.income-item.expense}});
  ...
  const expenseCategories = [
    ...
    { category: "Entertainment", percentage: 10, fill: "var(--color-entertainment)" },
  ];
  ...
  return (
    <div className="grid grid-cols-[1fr_10fr_1fr] w-full min-h-[55vh] py-4">
      <div className="col-start-2 col-span-1 h-full w-full gap-8 flex flex-col">
        <h1 className="text-5xl font-bold flex justify-center">Let's get banking, User.</h1>
                  ...
                  <AccordionItem value="item-4">
                    ...
                    <AccordionContent>
                      <Button asChild variant="ghost">
                        <Link to="/apply-for-loan" className="[&.active]:font-bold">
                          Apply for Loan
                        </Link>
                      </Button>
                    </AccordionContent>
          ...
          <div className="flex flex-col gap-4 w-3/4">
            <div className="flex flex-row gap-4">
              ...
              <Card className='w-1/3'>
                ...
                <CardFooter>
                  <p className="text-xs text-gray-500">+15.6% from last month</p>
                </CardFooter>
              </Card>
            </div>
            <Card className="">
              <CardHeader>
                ...
                <CardDescription>Get an overview of your financial situation in the past year.</CardDescription>
              </CardHeader>
              <CardContent>
                <Tabs defaultValue="incomeAndExpense" className="">
                  <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="incomeAndExpense">Income & Expense</TabsTrigger>
                    <TabsTrigger value="absoluteIncome">Absolute Income</TabsTrigger>
                  </TabsList>
                  ...
                  <TabsContent value="absoluteIncome">
                    <ChartContainer config={chartConfigAI}>
                      <BarChart accessibilityLayer data={AI}>
                        ...
                        />
                        <Bar dataKey="income">
                          <LabelList className="fill-background" position="insideTop" dataKey="month" fillOpacity={1} formatter={(value)=>value.slice(0,3)} />
                          {AI.map((item) => (
                            <Cell
                              key={item.month}
                              fill={
            ...
            <div className="flex flex-row gap-4">
              <Card className="flex flex-col w-1/2">
                <CardHeader>
                  ...
                  <CardDescription>What did you spend money on?</CardDescription>
                </CardHeader>
                    ...
                    <PieChart>
                      ...
                      <Pie data={expenseCategories} dataKey="percentage">
                        <LabelList
                          ...
                          stroke="none"
                          fontSize={12}
                          formatter={(value: keyof typeof chartConfigExpenseCategories) =>
                            value
                          }
                        />
                      </Pie>
              ...
              <Card className="flex flex-col w-1/2">
                ...
                <CardContent>
                  ...
                  >
                    <BarChart
                      accessibilityLayer
                      data={expenseCategories.sort((a, b) => b.percentage - a.percentage)}
                      layout="vertical"
                      margin={{
                        left: 35,
                      }}
                    >
                      <YAxis
                        ...
                        tickMargin={10}
                        axisLine={false}
                        tickFormatter={(value: keyof typeof chartConfigExpenseCategories) =>
                          value
                        }
                      />
