...
import { verifyAuthenticationPresentation } from "./verifyPresentation";
// import { hydraAdmin } from "@/config/ory";
import { isTrustedPresentation, extractClaims } from "./extractClaims";
import * as jose from "jose";
import { keyToDID, keyToVerificationMethod } from "@spruceid/didkit-wasm-node";
import { generatePresentationDefinition } from "./generatePresentationDefinition";
import { getConfiguredLoginPolicy } from "../config/loginPolicy";
import { withLogging } from "@/middleware/logging";
import { logger } from "../config/logger";

//TODO: function that generates random uuid and sends the login page
...
async function handler(req: NextApiRequest, res: NextApiResponse<any>) {
  try {
    ...
    if (method === "GET") {
      ...
      // TODO: return / use extracted information to fill form / check in database
